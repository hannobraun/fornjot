...
impl Context for HashMap<String, String> {
    fn get_argument(&self, name: RStr<'_>) -> ROption<RStr<'_>> {
        self.get(&*name).map(|s| s.as_str().into()).into()
    }
}
...
impl<C: Context + ?Sized> ContextExt for C {
    fn get_required_argument(
        ...
        match self.get_argument(name.into()) {
            ROption::RSome(v) => Ok(v.as_str()),
            ROption::RNone => Err(MissingArgument {
                name: name.to_string(),
            }),
        }
    }
    ...
    where
        ...
        T::Err: std::error::Error + Send + Sync + 'static,
    {
        let value = match self.get_argument(name.into()) {
            ROption::RSome(value) => value,
            ROption::RNone => return Ok(None),
        };
