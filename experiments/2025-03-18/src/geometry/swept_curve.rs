...
impl SurfaceGeometry for SweptCurve {
    ...
    fn approximate(
        ...
        let u = self.u.floating.geometry.as_ref();
        let v = self.v.geometry.as_ref();

        let [size_hint_u, size_hint_v] =
            (boundary.max - boundary.min).components;

        let [mut approx_u, mut approx_v] = [(u, size_hint_u), (v, size_hint_v)]
            .map(|(curve, size_hint)| {
        ...
        let mut current_u = None;
        while let Some(new_u) = approx_u.expand_to_include(min_u) {
            current_u = Some(new_u);
        }

        loop {
            let next_u = {
                let Some(current_u) = current_u else {
                    break;
                };
                if current_u > max_u {
                    break;
                ...
                let increment_u =
                    u.increment_at(current_u, tolerance, size_hint_u);

                current_u + increment_u.inner
            };

            current_u = approx_u.expand_to_include(next_u);
        }
        loop {
            if approx_v
